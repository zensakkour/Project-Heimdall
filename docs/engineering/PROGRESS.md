...
## 2026-05-12
- Hypothesis: Operator sessions are volatile and investigating nearby street-level context requires workflow improvements without breaking the platform style.
- Change: Implemented persistent local operator session logs (`operator_sessions/`) and added a fallback-capable `LocalStreetViewProvider`.
- Files touched: `src/tools/ui_server.py`, `src/dashboard/analysis/index.html`, `src/dashboard/analysis/operator.js`, `src/core/geo/street_view.py`.
- Validation command: `python -m pytest src/tests/test_ui_server_operator_session.py` and `python -m pytest src/tests/test_street_view.py`
