...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument(
        "--model-ids",
        default="openai/clip-vit-large-patch14,google/siglip-base-patch16-224",
    ...
    ok_rows = [row for row in results if row.get("status") == "ok" and row.get("median_km") is not None]
    ok_rows.sort(key=lambda row: float(row["median_km"]))
    payload = {
        ...
        "seed": int(args.seed),
        "models": results,
        "best_model": ok_rows[0]["model_id"] if ok_rows else None,
        "ranked_by_median_km": [row.get("model_id") for row in ok_rows],
    }
